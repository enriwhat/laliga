...
```{r}
#Limpieza de datos de las columnas quitando los espacios y cambiandolos por guiones ya que los espacios dan bastantes problemas a la hora de llamar a las variables
...
```{r}
# Creación de más campos que se quieren estudiar conforme avance el proyecto
...
```{r}
# Convertimos de variables a categorias
...
-   **total_yellow_cards**: segundas tarjetas amarillas totales
...
-   **Home Team**: Equipo local
-   **Away Team**: Equipo visitante
-   **Score**: Resultado (eliminado en la fase de limpiado)
-   **Half Time Score**: Resultado al descanso (eliminado en la fase de limpiado)
...
```{r}
# Sacamos los equipos que ha jugado cada equipo (más años en primera division)
