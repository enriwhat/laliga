...
train <- read.csv("train/train_laliga.csv")
test <- read.csv("test/test_laliga.csv")
```
...
str(train)
summary(train) #medias y medianas son bastante similares, asi que se pueden analizar como normales

train$Home.Team <- as.factor(train$Home.Team) #convertirlo a categorias
train$Away.Team <- as.factor(train$Away.Team)

train$Score <- NULL
train$Half.Time.Score <- NULL
train$Match.Excitement <- NULL
train$Home.Team.Rating <- NULL
train$Away.Team.Rating <- NULL

table(train$Home.Team)
prop.table(table(train$Home.Team))

table(train$Home.Team,train$Away.Team)

hist(train$Home.Team.Goals.Scored,breaks=4)
hist(train$Away.Team.Goals.Scored,breaks=4)
```
