...
```{r}
# tengo dudas de donde poner esto !!!
...
```{r}
# Esto es provisional seguro lo borro!!!

library(plotrix)
a <- train %>% group_by(score) %>% count(score)
b <- as.double(prop.table(table(train$score))*100,2)
datos <- a$n
...
```{r}
ggplot(train, aes(x = Away_Team, color = score, fill = score)) + geom_bar() + facet_wrap(~score) + coord_flip() + labs(x = "Equipo", y = "1    X    2", title = "Nº de   Victorias  -  Empates  -  Derrotas  como EQ.")+ theme(plot.title = element_text(hjust = 0.5))
```
