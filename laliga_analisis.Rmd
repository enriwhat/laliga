...
laliga$score <- as.factor(laliga$score)
...
t((prop.table(with(train, table(score, Home_Team)), margin = 2)))*100
t((prop.table(with(train, table(score, Away_Team)), margin = 2)))*100
...
```{r}
ggplot(train,aes(score ,Home_Team_Goals_Scored)) +
  geom_bar(position = "dodge", stat = "summary", fun = "mean",
           color = 'black', fill = 'green') +
