...
```{r}
reg_pl_2 <- lm(Petal.Length ~ Sepal.Length, data = iris)
summary(reg_pl_2)
```
...
R cuadrado es 0.76 indica que el 76% de la variabilidad en la longitud del pétalo puede ser explicada por la variabilidad en la longitud del sépalo. Vemos que la asociación entre la longitud del pétalo y la longitud del sépalo es estadísticamente significantiva por su p valor.
...
```{r}
plot(reg_pl_2$residuals ~ iris$Petal.Length, xlab = "L. Pétalo", ylab = "Residuals")
