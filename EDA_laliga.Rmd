...
ggcorrplot(cor(num_data), lab=FALSE)
```
...
Hay una relación también evidente (y son las más fuertes del dataset) entre los distintos tipos de tiros (cuanto más tiras, más disparas a puerta y también más fuera).

```{r}
plot_grid(qplot(Home_Team_Total_Shots, Home_Team_On_Target_Shots, data = train) +
  geom_smooth(method = "lm") + 
    labs(x="Tiros", y="Tiros a puerta") +
  ggtitle("Relación entre los tiros totales y los tiros a puerta"),
  
  qplot(Home_Team_Total_Shots, Home_Team_Off_Target_Shots, data = train) +
  geom_smooth(method = "lm") + 
    labs(x="Tiros", y="Tiros a puerta") +
  ggtitle("Relación entre los tiros totales y los tiros a puerta"),
  
  qplot(Home_Team_Total_Shots, Home_Team_Blocked_Shots, data = train) +
  geom_smooth(method = "lm") + 
    labs(x="Tiros", y="Tiros a puerta") +
  ggtitle("Relación entre los tiros totales y los tiros a puerta"),
nrow = 1)
...
plot_grid(qplot(Home_Team_Total_Shots, Home_Team_On_Target_Shots, data = train) +
  ...
  qplot(sqrt(Home_Team_Total_Shots), sqrt(Home_Team_On_Target_Shots), data = train) +
  geom_smooth(method = "lm") + 
    labs(x="Tiros", y="Tiros a puerta") +
  ggtitle("Relación entre los tiros totales y los tiros a puerta"),
  
  qplot(log10(Home_Team_Total_Shots), log10(Home_Team_On_Target_Shots), data = train) +
  geom_smooth(method = "lm") + 
    labs(x="Tiros", y="Tiros a puerta") +
  ggtitle("Relación entre los tiros totales y los tiros a puerta"),
nrow = 2)
...
```{r}
plot_grid(ggplot(train, aes(Home_Team_Goals_Scored)) + geom_bar(fill="#d55566") +
...
```{r}
train %>% mutate(log10_ppgdp = sqrt(Home_Team_Goals_Scored)) %>%
  select(log10_ppgdp) %>%
  na.omit() %>%
  ggplot(aes(x=log10_ppgdp)) +
    geom_density()
```
