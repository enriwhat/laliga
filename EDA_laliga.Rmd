...
Sin embargo en el caso de los goles y las expulsiones no parece tan evidente que la distribución sea normal (más bien parecen exponenciales). Para comprobarlo recurrimos a un gráfico Q-Q Plot.

# Análisis multivariante

Una vez analizadas todas las variables de forma individual, vamos a buscar las relaciones que hay entre ellas en los siguientes gráficos.
...
Como se ha observado en la parte de visualización, la gran mayoría de las variables numéricas se distribuyen como una normal. Además todas se encuentran en una escala similar de valores, por lo que transformarlas no tendría mucho sentido.
...
En cambio, si realizamos el mismo test para los goles, el resultado no es tan claro:
...
Incluso se compara mejor a una exponencial, como se ve en el Q-Q Plot inferior:
...
Para el caso de las variables relacionadas con las expulsiones (tarjetas rojas y dobles amarillas), al tener tan pocos valores (no es habitual que haya más de una expulsión en un equipo), se ha optado por binalizarlas (expulsión/es o no expulsión/es).
...
Una vez hecho podemos visualizar para comprobar que efectivamente, son poco habituales las expulsiones en un partido, aunque ligeramente mayor en el equipo visitante.
...
```{r}
ggplot(train, aes(expulsion, fill = score)) +
  geom_bar() +
  labs(x = "Expulsiones", y = "Partidos", title = "Nº de expulsiones") +
  theme(plot.title = element_text(hjust = 0.5))
```
